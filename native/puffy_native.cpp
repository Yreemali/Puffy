...
#elif defined(PUFFY_HAS_WASAPI_BACKEND)
#include "platform/windows/windows_audio_backend.hpp"
#elif defined(PUFFY_HAS_COREAUDIO_BACKEND)
...
#endif
#if defined(PUFFY_HAS_WASAPI_BACKEND)
...
#elif defined(PUFFY_HAS_WASAPI_BACKEND)
    puffy::platform::windows::WasapiCapture capture;
    puffy::platform::windows::WasapiOutput output;
    puffy::platform::windows::WindowsVirtualMicrophone microphone{
        std::getenv("PUFFY_WINDOWS_VIRTUAL_TRANSPORT_ID") != nullptr
            ? std::getenv("PUFFY_WINDOWS_VIRTUAL_TRANSPORT_ID") : ""};
#elif defined(PUFFY_HAS_COREAUDIO_BACKEND)
    puffy::platform::macos::CoreAudioCapture capture;
    puffy::platform::macos::CoreAudioOutput output;
    puffy::platform::macos::MacOSVirtualMicrophone microphone{
        std::getenv("PUFFY_MACOS_VIRTUAL_TRANSPORT_UID") != nullptr
            ? std::getenv("PUFFY_MACOS_VIRTUAL_TRANSPORT_UID") : ""};
#endif
#if defined(PUFFY_HAS_GLOBAL_HOTKEYS)
    std::unique_ptr<puffy::hotkeys::IGlobalKeyboardListener> keyboardListener;
    std::unique_ptr<puffy::hotkeys::HotkeyRouter> hotkeys;
#endif
    std::string error;
    std::string snapshot;
    std::string transientJson;
...
#if defined(PUFFY_HAS_GLOBAL_HOTKEYS)
#if defined(PUFFY_HAS_WASAPI_BACKEND)
    context->keyboardListener = std::make_unique<puffy::platform::windows::WindowsGlobalKeyboardListener>();
#elif defined(PUFFY_HAS_COREAUDIO_BACKEND)
    context->keyboardListener = std::make_unique<puffy::platform::macos::MacOSGlobalKeyboardListener>();
#else
...
extern "C" int puffy_native_start(puffy_native_context* context) {
    if (!context) return 0;
#if defined(PUFFY_HAS_PIPEWIRE) || defined(PUFFY_HAS_WASAPI_BACKEND) || defined(PUFFY_HAS_COREAUDIO_BACKEND)
    if (context->started) return 1;
    if (!context->engine.start(context->capture, context->output, &context->microphone)) {
        context->error = "Unable to start the platform audio graph";
        return 0;
    }
...
extern "C" int puffy_native_set_master_volume(puffy_native_context* context, float value) {
    ...
    context->engine.setMasterGain(value);
    return 1;
}
...
extern "C" const char* puffy_native_sound_waveform(puffy_native_context* context,
                                                    int64_t id,
                                                    size_t points) {
    if (!context) return "[]";
    const auto peaks = context->service.waveform(id, points);
    std::ostringstream output;
    output << '[';
    ...
    output << ']';
    context->transientJson = output.str();
    if (peaks.empty()) context->error = context->service.lastError();
    return context->transientJson.c_str();
}

extern "C" const char* puffy_native_mixer_levels(puffy_native_context* context) {
    if (!context) return "{}";
    const auto levels = context->engine.mixer().levels();
    std::ostringstream output;
    output << "{\"microphone\":" << levels.microphone
           << ",\"soundboard\":" << levels.soundboard
           << ",\"monitoring\":" << levels.monitoring
           << ",\"virtualOutput\":" << levels.virtualOutput << '}';
    context->transientJson = output.str();
    return context->transientJson.c_str();
}
